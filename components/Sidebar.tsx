...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange }) => {
  ...
  return (
    <>
          ...
          {items.map((item) => (
            <button
              ...
              className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 border ${
                currentView === item.id 
                  ? 'bg-white/20 backdrop-blur-sm border-white/30 text-white shadow-lg font-semibold' 
                  : 'border-transparent hover:bg-white/10 text-blue-100'
              }`}
            >
              <i className={`fa-solid ${item.icon} w-5 text-center`}></i>
              <span>{item.label === 'Home' ? 'Dashboard' : item.label === 'Cards' ? 'Flashcards' : item.label === 'Plan' ? 'Study Planner' : item.label === 'AI' ? 'AI Tutor' : item.label === 'FAQs' ? 'FAQ Matrix' : item.label === 'Simple' ? 'Simplifier' : item.label}</span>
            </button>
          ))}
        </nav>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-sjsu-blue text-white z-50 flex items-center justify-around px-2 py-3 border-t border-blue-800 shadow-[0_-4px_12px_rgba(0,0,0,0.1)]">
        {items.map((item) => (
          <button
            key={item.id}
            onClick={() => onViewChange(item.id)}
